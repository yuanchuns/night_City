...
- (void)callBeginPullDownRefreshing {
    if ([self.delegate respondsToSelector:@selector(beginPullDownRefreshing)]&& self.isPullDownRefreshed) {
        [self.delegate beginPullDownRefreshing];
   }
}
//上拉加载更多开始 <调用上拉加载更多协议>
- (void)callBeginPullUpLoading {
    if ([self.delegate respondsToSelector:@selector(beginPullUpLoading)] && self.isPullUpLoadMore) {
...
- (BOOL)isPullDownRefreshed {
    ...
    if([self.delegate respondsToSelector:@selector(hasRefreshHeaderView)]){
       ...
       BOOL hasHeadView = [self.delegate hasRefreshHeaderView];
        return hasHeadView;
    }
    return YES;
}
...
- (void)resetScrollViewContentInsetWithDoneLoadMore {
    ...
    [UIView animateWithDuration:0.3f animations:^{
        ...
        if (_refreshFooterView) {
            ...
            if (self.isNotMore) {
                self.loadMoreState = CLLLoadMoreStateStopped;
            }else{
                self.loadMoreState = CLLLoadMoreStateNormal;
            }
            CGRect tmpFrame = _refreshFooterView.frame;
            tmpFrame.origin.y = (self.scrollView.contentSize.height==0)?self.scrollView.bottom:self.scrollView.contentSize.height;
...
- (void)setLoadMoreState:(CLLLoadMoreState)loadMoreState {
    ...
    switch (loadMoreState) {
        case CLLLoadMoreStateStopped://表示所有的都加载已经加载完毕
        {
           
             self.refreshFooterView.statusLabel.text = @"没有更多了哟";
...
- (void)setRefreshState:(CLLRefreshState)refreshState
{
    switch (refreshState) {
        ...
        case CLLRefreshStateNormal: {
          if(self.isPullUpLoadMore)   self.refreshFooterView.hidden = NO;
            self.refreshHeadView.statusLabel.text = @"下拉刷新";
            [self.refreshHeadView.circleView stopAnimation];
            break;
        }
        case CLLRefreshStateLoading: {
            if (self.pullDownRefreshing) {
                self.refreshHeadView.statusLabel.text = @"正在加载";
                [self.refreshHeadView.circleView startAnimation];
                [self setScrollViewContentInsetForLoading];
              if(self.isPullUpLoadMore) self.refreshFooterView.hidden = YES;
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if ([keyPath isEqualToString:@"contentOffset"]) {
        ...
        if (self.isPullDownRefreshed) {
          ...
          if (self.isPullUpLoadMore && !self.isNotMore && contentOffset.y+self.scrollView.height > self.scrollView.contentSize.height && contentOffset.y != 0) {
            if(self.loadMoreState != CLLLoadMoreStateLoading && !self.pullDownRefreshing ) {
        ...
        if (hasFooterView ) {
            ...
            tmpFrame.origin.y = self.scrollView.contentSize.height>0?self.scrollView.contentSize.height:self.scrollView.bottom;
