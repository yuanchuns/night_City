...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    // 进来先清空存的第一个密码
    [PCCircleViewConst saveGesture:nil Key:MGGestureOneSaveKey];
...
- (void)setupSubViewsLoginVc
{
    ...
    if ([kUserDefaults objectForKey:kMGLoginNameKey]) {
        [imageView sd_setImageWithURL:[NSURL URLWithString:getImaUrlWithName(kMGLoginNameKey)] placeholderImage:[UIImage imageNamed:kMGHeadImaName]];
//        imageView.image = [UIImage imageNamed:[kUserDefaults objectForKey:kMGHeadImaNameKey]];
    }else {
        
        imageView.image = [UIImage imageNamed:kMGHeadImaName];
    }
...
- (void)didClickBtn:(UIButton *)sender
{
    ...
    switch (sender.tag) {
        ...
        case buttonTagForget:{
            ...
            T2TNavController *nav = getNavWithVc(lvc);
...
- (void)circleView:(PCCircleView *)view type:(CircleViewType)type connectCirclesLessThanNeedWithGesture:(NSString *)gesture
{
    ...
    NSString *gestureOne = [PCCircleViewConst getGestureWithKey:MGGestureOneSaveKey];
...
- (void)circleView:(PCCircleView *)view type:(CircleViewType)type didCompleteSetSecondGesture:(NSString *)gesture result:(BOOL)equal
{
    ...
    if (equal) {
        ...
        [PCCircleViewConst saveGesture:gesture Key:MGGestureFinalSaveKey];
