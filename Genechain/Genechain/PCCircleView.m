...
- (void)gestureEndByTypeSettingWithGesture:(NSString *)gesture length:(CGFloat)length
{
    if (length < CircleSetCountLeast) {
        ...
        if ([gestureOne length] < CircleSetCountLeast) { // 接收并存储第一个密码
            // 记录第一次密码
            [PCCircleViewConst saveGesture:gesture Key:MGGestureOneSaveKey];
            ...
            BOOL equal = [gesture isEqual:[PCCircleViewConst getGestureWithKey:MGGestureOneSaveKey]]; // 匹配两次手势
...
{
    NSString *password = [PCCircleViewConst getGestureWithKey:MGGestureFinalSaveKey];
